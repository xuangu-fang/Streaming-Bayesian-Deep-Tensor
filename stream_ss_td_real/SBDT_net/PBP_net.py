...
class PBP_net:
    ...
    def pbp_train(self,X_test,y_test,help_str=''):
        ...
        if self.mode == 'single':
            ...
            while count+mini_batch<=self.X_train.shape[0] and turn<100:
                ...
                if (turn%self.test_point ==0):
                    ...
                    auc = np.mean((y_test - m)**2)
                    self.running_score.append(auc)
                    print('after %d th batch(%.3f), the score is %.4f'%(turn,float(turn)/self.N_turns,auc ))
